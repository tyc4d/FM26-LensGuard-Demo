...
class DemoRuntime:
    ...
    def start(self, scenario: Scenario, guard_enabled: bool, frame: FrameInput | None = None) -> RunState:
        ...
        if self.closing or active >= self.settings.max_concurrent_runs:
            raise RuntimeCapacityError("The demo runtime is busy. Retry after the active run completes.")
    ...
    async def _run(self, record: RunRecord, scenario: Scenario, frame_id: str, frame: FrameInput | None = None) -> None:
        ...
        try:
            ...
            if isinstance(self.provider, PrototypeRuntimeProvider):
                if frame is None:
                    raise RuntimeFailure("Image missing. Start the camera or upload an image.")
                await self.provider.run(state, frame, lambda kind, detail: self._publish(record, kind, detail))
                return
            await self._publish(record, "frame.received", f"Mock frame fixture {frame_id}; browser video stays local.")

            await self._pause()
            state.interpretation = await self.provider.analyze_scene(scenario)
            await self._publish(record, "perception.scene_analyzed", "Scene interpretation loaded from the selected mock scenario.")

            await self._pause()
            state.regions = await self.provider.extract_regions(scenario)
            await self._publish(record, "perception.text_extracted", f"{len(state.regions)} camera-source regions from mock fixture.")

            await self._pause()
            state.action = await self.provider.propose_action(scenario, state.id)
            arguments = ", ".join(f'{name}="{value.value}"' for name, value in state.action.arguments.items())
            await self._publish(record, "model.action_proposed", f"{state.action.tool}({arguments}) proposed by mock model.")
            ...
            await self._publish(record, "provenance.attached", "Camera lineage attached" + (" with explicit user delegation." if scenario.explicit_delegation else "; authority remains observation-only."))
            ...
            if state.guard_enabled:
                ...
                detail = f"{policy_label}: {state.decision.affected_argument} requires {state.decision.required_authority}."
            else:
                policy_label = "BYPASS"
                event_type = "policy.bypassed"
                detail = "LensGuard is OFF; authorization gate bypassed for simulation."
            state.trace_nodes.append(TraceNode(id="policy", label=policy_label, type="policy decision", source="system"))
            ...
            state.trace_nodes.append(TraceNode(id="outcome", label=state.outcome.status.upper(), type="simulation outcome", source="system"))
            ...
            state.status = "failed"
            state.error = "Runtime analysis failed. Reset the demo and retry; consult backend logs for details."
            await self._publish(record, "runtime.failed", state.error)

    async def _cancelled(self, record: RunRecord) -> None:
        record.state.status = "failed"
        record.state.error = "Backend stopped before this run completed. Start a new analysis after reconnecting."
