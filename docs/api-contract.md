...
- `POST /api/run`: returns HTTP 202 initial `RunState`. Mock accepts existing JSON `{scenario_id, guard_enabled}`. Real requires multipart `image` (JPEG/PNG, <=10 MiB), `user_request` (1–4000 characters), `scenario_id`, `guard_enabled` (`true`/`false`), optional `source` (`camera`/`uploaded_image`), `capture_ms`, and `model_profile` (`nemotron-nano-vl-8b` or `cosmos-reason1-7b`). Omission uses the gateway default, Nemotron. Other selections and model selection in Mock mode return HTTP 422.
...
The host [NVIDIA gateway](nvidia-demo.md) at `PROTOTYPE_RUNTIME_URL` accepts the same `/v1/analyze` multipart contract plus the optional `model_profile` field. The bridge forwards it unchanged and rejects a different returned `model.profile` with `model_mismatch`, without an action or outcome. The requested profile is present in every initial/SSE snapshot; completed responses retain the actual profile under both `RunState.model_profile` and `runtime_metadata.model.profile`.

Gateway `/health` adds `models: [{id, name, available}]` and `default_model`, forwarded under `/api/health`'s `prototype`. The catalog contains only Nemotron and Cosmos. `available` checks the executable and server directory, not whether a future GPU load will succeed. Existing `model_profile`, `model_id`, `model_loaded`, and GPU telemetry describe the currently resident worker. Browser preferences are independent of the currently resident model; polling does not overwrite a visitor's selection.

Changing the selector makes no inference request. Both comparison submissions freeze the selected model alongside the same image and user request, and the frontend also rejects a comparison response with a different profile. One gateway-owned worker runs at a time; concurrent gateway inference returns HTTP 409. When forwarded through the asynchronous Demo API, this becomes a terminal failed run. A missing environment, load error, timeout, or mismatch does not substitute another model or Mock data. The selector is disabled during both halves of comparison and while disconnected.
...
The Demo UI, scenario descriptions, public errors, authorization explanations and event details use English (`en`). API enum values, identifiers, tool/argument keys and submitted user requests retain their existing contract. The UI translates known field/tool/status labels without modifying their stored values.

Original model/parser content and `runtime_metadata.policy.reason` remain unchanged. Public copies of upstream errors are localized; original transport diagnostics are retained in `runtime_metadata.upstream_error`, and a health error retains its original text in `prototype.raw_error`. Both are available under the collapsed **Original service errors** disclosure, including failures before any model output exists.
